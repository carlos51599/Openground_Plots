#!/usr/bin/env python3
"""
Multi-Source Parameter Plotting System - Unit Weight Bulk vs Depth
...
sample size for each investigation. Additionally, a total count "Total n=xxx"
appears at the bottom of the legend.

IMPLEMENTATION:
- Line ~2795: Calculate investigation_counts dictionary
- Line ~2876: Add count to regular trace legend names
- Line ~2968: Add count to dummy circle trace legend names
- Line ~2974: Add total count trace to legend
...
Responsibility:
Generates unit weight bulk vs depth scatter plots by consolidating data from multiple CSV sources
...
CONFIG = {
    ...
    # ═══════════════════════════════════════════════════════════════════════
    "parameter": {
        "name": "UnitWeightBulk",  # MODIFICATION POINT: Change to any parameter
        "display_name": "Unit Weight Bulk (kN/m³)",  # For axis labels
...
def apply_empirical_calculations(
    ...
    # Density to Unit Weight Bulk conversion (ρ to γ)
    # Conversion formula: γ = ρ × g, where g = 9.81 m/s²
    # Input: Density (Mg/m³) → Output: Unit Weight Bulk (kN/m³)
    # NOTE: All columns mapped to UnitWeightBulk parameter contain density values in Mg/m³
    #       that require conversion. No heuristic checking - use exact parameter match.
    if parameter_name == "UnitWeightBulk":
        logger.info(
            f"🔄 Converting Density to Unit Weight Bulk for {csv_name} (column: {column_name})"
        )

        # Acceleration due to gravity (m/s²)
        g = 9.81

        # Apply conversion: γ (kN/m³) = ρ (Mg/m³) × g (m/s²)
        # Note: 1 Mg/m³ = 1000 kg/m³, and γ = ρ × g = 1000 kg/m³ × 9.81 m/s² / 1000 N/kN = ρ (Mg/m³) × 9.81 kN/m³
        df_copy[column_name] = pd.to_numeric(df_copy[column_name], errors="coerce") * g

        # Count successful conversions
        valid_conversions = df_copy[column_name].notna().sum()
        logger.info(
            f"✅ Density to Unit Weight conversion completed: {valid_conversions} values converted (γ = ρ × {g})"
        )
...
def load_parameter_data(
    ...
    # Apply empirical calculations and unit conversions
    # MODIFICATION POINT: This processes each loaded CSV before grouping by formation
    # Always runs for unit conversions (e.g., Density to Unit Weight)
    # Also runs for empirical calculations if configured (e.g., SPT N-value to cu)
    if csv_data_dict:
        logger.info("")
        logger.info(f"🔧 Applying empirical calculations and unit conversions...")

        # Extract configuration values (coordination boundary)
        # Use .get() to handle optional empirical_calculations config
        f1_factor_config = CONFIG.get("empirical_calculations", {}).get(
            "spt_to_cu_factors", None
        )
        geology_code_descriptions = CONFIG["mappings"]["geology_code_descriptions"]
        ...
        logger.info(f"✅ Empirical calculations and unit conversions complete")
        logger.info("")
            ...
            if len(regular_data) > 0:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        name=legend_name,  # Investigation name in legend with count
