#!/usr/bin/env python3
"""
Multi-Source Parameter Plotting System - Plasticity Index vs Depth Plots
AI-Optimized Monolith Implementation - All Phases Complete

Architectural Overview:

Responsibility:
Generates Plasticity Index vs Depth plots by consolidating Plasticity Index data from multiple
CSV sources (test types). The system is configurable and automatically identifies and combines
all relevant test types from the global parameter mapping CSV. Creates investigation-series
depth plots that color by investigation while using marker shapes for test types.

Key Interactions:
- Input: Global parameter mapping CSV + geological data CSVs (Classification by Geology) + Location Details CSV
- Configuration: Single parameter selection (PlasticityIndex), color schemes, geological mappings
- Processing: Mapping extraction → CSV loading → formation grouping → outlier filtering
  → Depth plot generation (colors by investigation, shapes by test type) →
  investigation tracking (CSV summaries) → Excel export with highlighted outliers
- Output: Investigation-series depth plots + investigation CSV summaries + Excel exports
  in Output/PlasticityIndex/investigation_series/ and Output/PlasticityIndex/data/
- Output Control: Three-level hierarchy (Master → Category → Individual) for selective output

Navigation Guide:
Data flows: Parameter mapping extraction (PHASE 1: PI) → CSV data loading (PHASE 2) →
Formation grouping (PHASE 3) → Outlier filtering (PHASE 3b) →
Depth plot generation (PHASE 4: depth vs parameter) → Investigation tracking (PHASE 5) →
...
- Generates depth vs Plasticity Index plots with investigation-based coloring and test-type marker shapes
- Uses single parameter: Plasticity Index (X-axis) vs Depth (Y-axis, inverted)
...
- Publication-quality charts (300 DPI, standard depth plot aspect ratio)
- X-axis: Plasticity Index (%), Y-axis: Depth (m, inverted)
- Output: {output_folder}/PlasticityIndex/investigation_series/{with/without_outliers}/{formation_name}.png
...
- Organizes by test type: {test_type}.xlsx (e.g., Classification.xlsx)
...
CONFIG = {
    ...
    # ═══════════════════════════════════════════════════════════════════════
    "parameter": {
        "name": "PlasticityIndex",  # MODIFICATION POINT: Single parameter analysis (PI vs Depth)
        "display_name": "Plasticity Index (%)",  # For X-axis label
    ...
    "plotting": {
        "figure": {
            "width": 10,  # Depth plot width
            "height": 8,  # Depth plot height
            "dpi": 300,
        },
        "colors": {
        ...
        "axes": {
            "x_label_position": "top",  # X-axis label at top for depth plots
            "x_label_size": 10,
            "x_label_weight": "normal",
            "y_label": "Depth (m)",  # Y-axis label for depth
            "y_label_size": 10,
            "y_label_weight": "normal",
            "x_limit_left": 0,  # X-axis starts at 0
            "y_margin": 0.5,  # Margin for Y-axis (depth)
            "invert_y": True,  # Invert Y-axis (depth increases downward)
            "grid": True,
            "grid_which": "both",
            "grid_style": "-",
            "grid_width": 0.5,
        ...
        "title": {
            "fontsize": 14,
            "weight": "normal",
            "pad": 15,  # Standard padding for depth plots
        },
        "legend": {
    ...
    # ═══════════════════════════════════════════════════════════════════════
    "csv_source_settings": {
        # Classification by Geology - Primary source for Plasticity Index
...
def group_data_by_formation(
    ...
    Processing Logic:
    1. For each CSV, lookup parameter column name
    2. Verify column exists in DataFrame
    3. Group CSV data by Geological_Strata values
    4. Filter out rows with null parameter or depth values
    ...
    for csv_name, df in csv_data_dict.items():
        ...
        param_column = matching_mappings["column_name"].iloc[0]
        param_name = matching_mappings["parameter"].iloc[0]

        # === COLUMN VALIDATION SECTION ===
        # Verify parameter column exists in DataFrame
        if param_column not in df.columns:
            logger.warning(
                f"⚠️ {csv_name}: Parameter column '{param_column}' not found in data, skipping"
            )
            continue

        # Verify required columns exist
        required_cols = ["Geological_Strata", "Top Depth", param_column]
        ...
        for formation in formations:
            ...
            # Remove rows with null parameter or depth values
            formation_data = formation_data.dropna(subset=[param_column, "Top Depth"])

            # === STORAGE SECTION ===
            if not formation_data.empty:
                formation_dict[formation][csv_name] = formation_data
                logger.debug(
                    f"    → {formation}: {len(formation_data)} samples from {csv_name} ({param_name}={param_column})"
                )
...
def _add_investigation_column(
    ...
    if (
        ...
        or "Location ID" not in csv_data.columns
        or "Investigation" not in location_details.columns
    ):
        # No location details, no Location ID column, or no Investigation column - use fallback
    ...
    if "Investigation" in result.columns:
...
def generate_investigation_series_depth_plot(
    formation_name: str,
    csv_dict: Dict[str, pd.DataFrame],
    parameter_mappings: pd.DataFrame,
    parameter_display_name: str,
    ...
    """
    Generate depth vs parameter plot with investigation-based coloring and test-type shapes.

    Creates depth plot where:
    - Colors represent different investigations (rotating palette)
    - Markers/shapes represent different test types (from CONFIG)
    - X-axis: Parameter values (%), Y-axis: Depth (m)
    ...
    Output:
        Saves PNG: {output_folder}/{formation_name_sanitized}.png
    """
    logger.info(f"🎨 Generating depth plot for formation: {formation_name}")
    ...
    # Track unique investigations for legend
    investigations_plotted = set()
    test_types_plotted = set()

    # Prepare data: filter outliers first, THEN add Investigation column
    csv_data_with_investigations = {}
    all_investigations = set()
    ...
    for csv_name, df in csv_dict.items():
        ...
        if column_name not in df.columns:
            logger.warning(
                f"⚠️ {csv_name}: Parameter column '{column_name}' not found, skipping"
            )
            continue

        if "Top Depth" not in df.columns:
            logger.warning(f"⚠️ {csv_name}: 'Top Depth' column not found, skipping")
            continue

        # Apply outlier filtering FIRST
        if outlier_results and csv_name in outlier_results:
            outlier_indices = outlier_results[csv_name]["outlier_indices"]
            df_filtered = df[~df.index.isin(outlier_indices)]
        else:
        ...
        # Add investigation column AFTER filtering
        df_with_inv = _add_investigation_column(
            df_filtered, location_details, fallback_investigation_name
        )
    ...
    for csv_name, data_info in csv_data_with_investigations.items():
        ...
        # Get marker settings for this test type
        source_settings = CONFIG["csv_source_settings"].get(
            csv_name, CONFIG["default_source_settings"]
        )
        marker = source_settings.get("marker", "o")
        marker_size = source_settings.get("marker_size", 50)
        marker_alpha = source_settings.get("alpha", 0.7)

        # Get clean test type name for legend
        test_type_name = source_settings.get(
            "legend_name_points",
            csv_name.replace(" by Geology.csv", "").replace(".csv", ""),
        )

        # Plot each investigation separately with its color
        for investigation in df_with_inv["Investigation"].unique():
            ...
            # Get investigation color from mapping
            inv_color = investigation_color_map[investigation]

            # Plot with investigation color and test type marker
            ax.scatter(
                param_values,
                depth_values,
                c=inv_color,
                ...
                edgecolors="black",
                linewidths=0.5,
                label=f"{investigation}_{test_type_name}",  # Temporary label for tracking
            )

            all_parameter_values.extend(param_values)
            all_depth_values.extend(depth_values)
            investigations_plotted.add((investigation, inv_color))
            test_types_plotted.add((test_type_name, marker))
    ...
    ax.set_xlim(left=x_limit_left, right=max_param * 1.1)
    ...
    # === DUAL LEGEND (Investigations + Test Types) ===
    ...
    # Investigation colors (using circle markers)
    show_investigation_colors = CONFIG["investigation_series"]["legend"][
        "show_investigation_colors"
    ]
    if show_investigation_colors:
        for investigation, color in sorted(investigations_plotted):
            legend_handles.append(
                Line2D(
                    [0],
                    [0],
                    marker="o",  # Circle marker for investigation colors
                    color="w",
            ...
            legend_labels.append(investigation)

    # Test type shapes
    show_test_type_shapes = CONFIG["investigation_series"]["legend"][
        "show_test_type_shapes"
    ]
    if show_test_type_shapes:
        for test_type, marker in sorted(test_types_plotted):
            legend_handles.append(
                Line2D(
                    [0],
                    [0],
                    ...
                    markeredgecolor="black",
                    markersize=8,
                    label=f"{test_type} (shape)",
                )
            )
            legend_labels.append(f"{test_type} (shape)")
    ...
    legend_edgecolor = CONFIG["plotting"]["legend"].get("edgecolor", "black")

    # Get legend handles and labels
    num_entries = len(legend_labels)
    ...
    if legend_fixed_width:
        ...
        legend_location = CONFIG["plotting"]["legend"].get("location", "upper center")
        legend = ax.legend(
            legend_handles,
            legend_labels,
            loc=legend_location,
    ...
    logger.info(f"✅ Saved depth plot: {output_path}")


def generate_investigation_series_depth_plot_plotly(
    formation_name: str,
    csv_dict: Dict[str, pd.DataFrame],
    parameter_mappings: pd.DataFrame,
    parameter_display_name: str,
    ...
    """
    Generate interactive Plotly HTML version of Plasticity Index vs depth plot.
    ...
    - Hover tooltips showing: Depth, Plasticity Index, Location ID, Investigation name
    ...
    # Collect outlier data grouped by investigation (for later trace generation)
    ...
    for csv_name, data_info in sorted_csv_items:
        ...
        for investigation in sorted(df_with_inv["Investigation"].unique()):
            ...
            # Collect outlier points grouped by investigation (for later trace generation)
            if mark_outliers and len(outlier_data) > 0:
                # Initialize investigation in outlier dict if not exists
                ...
                # Add outlier data to this investigation's collection
    ...
    # === ADD OUTLIER TRACES (ONE PER INVESTIGATION) ===
    # Outliers have their own independent legendgroup "Outlier" for standalone toggle control
            ...
            if len(outlier_info["x"]) > 0:
                fig.add_trace(
                    go.Scatter(
                        ...
                        name="Outlier",
                        legendgroup="Outlier",  # Standalone legendgroup for all outliers
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            x=1.02,
            font=dict(size=10),
            groupclick="togglegroup",  # CRITICAL FIX: Toggles ALL traces in legendgroup (all points of same investigation color)
...
def generate_investigation_series_plots(
    ...
    for formation_name in sorted(formation_groups.keys()):
        ...
        if should_generate_plot(
            ...
            generate_investigation_series_depth_plot(
                formation_name,
                csv_dict,
                parameter_mappings,
                parameter_display_name,
        ...
        if should_generate_plot(
            ...
            generate_investigation_series_depth_plot_plotly(
                formation_name,
                csv_dict,
                parameter_mappings,
                parameter_display_name,
...
def main():
    """
    Main execution function - orchestrates all phases of Plasticity Index vs Depth plot generation.

    Implementation Status:
    - Phase 1: Parameter Mapping Extraction (PI) ✅ COMPLETE
    - Phase 2: CSV Data Loading ✅ COMPLETE
    - Phase 3: Formation Grouping ✅ COMPLETE
    - Phase 3b: Outlier Filtering ✅ COMPLETE
    - Phase 4: Depth Plot Generation ✅ COMPLETE
    - Phase 5: Investigation Source Tracking ✅ COMPLETE
    - Phase 6: Excel Export with Highlighted Outliers ✅ COMPLETE
    """
    logger.info("=" * 80)
    logger.info("🚀 Multi-Source Plasticity Index vs Depth Plot Generator v1")
    logger.info("=" * 80)
    ...
    try:
        ...
        # === PHASE 4: INVESTIGATION SERIES PLOT GENERATION ===
        logger.info("")
        # For depth plots, use display_name from CONFIG
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info("✅ Phase 4 Complete: Depth plots generated successfully")
        logger.info("=" * 80)
