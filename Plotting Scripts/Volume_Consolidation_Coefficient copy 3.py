#!/usr/bin/env python3
"""
Multi-Source Parameter Plotting System - Coefficient of Volume Compressibility vs Cell Pressure
AI-Optimized Monolith Implementation - All Phases Complete

Architectural Overview:

Responsibility:
Generates mv vs Cell Pressure charts by consolidating Coefficient of Volume Compressibility (mv)
...
- Configuration: Dual parameter selection (mv + Cell Pressure), color schemes, geological mappings
- Processing: Mapping extraction → CSV loading → formation grouping → outlier filtering
  → mv vs Cell Pressure plot generation (colors by investigation, shapes by test type) →
...
- Output: Investigation-series charts + investigation CSV summaries + Excel exports
  in Output/CoefficientVolumeCompressibility/investigation_series/ and Output/CoefficientVolumeCompressibility/data/
- Output Control: Three-level hierarchy (Master → Category → Individual) for selective output

Navigation Guide:
Data flows: Dual parameter mapping extraction (PHASE 1: mv + Cell Pressure) → CSV data loading (PHASE 2) →
Formation grouping (PHASE 3) → Outlier filtering (PHASE 3b) →
mv vs Cell Pressure plot generation (PHASE 4: standard scatter plot) → Investigation tracking (PHASE 5) →
...
CONFIG = {
    ...
    # ═══════════════════════════════════════════════════════════════════════
    "parameter": {
        "name": "CoefficientVolumeCompressibility",  # MODIFICATION POINT: Dual parameter analysis (mv + Cell Pressure)
        "x_parameter": "Cell Pressure",  # X-axis parameter
        "y_parameter": "CoefficientVolumeCompressibility",  # Y-axis parameter
        "x_display_name": "Cell Pressure (kPa)",  # For X-axis label
        "y_display_name": "mv (m²/MN)",  # For Y-axis label
