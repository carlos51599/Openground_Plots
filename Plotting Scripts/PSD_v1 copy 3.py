#!/usr/bin/env python3
"""
Multi-Source Parameter Plotting System - Particle Size Distribution
AI-Optimized Monolith Implementation - All Phases Complete

Architectural Overview:

Responsibility:
Generates particle size distribution plots by consolidating data from multiple CSV sources
(test types) for geotechnical grading analysis. The system plots averaged percentage passing
vs particle size (log scale) with one line per investigation. For each investigation, percentage
passing values are averaged at each particle size across all location IDs within that investigation.
The system is configurable and parameter-agnostic, automatically identifying and combining all relevant
test types for any parameter defined in the global parameter mapping CSV.

Key Interactions:
- Input: Global parameter mapping CSV + geological data CSVs (Grading by Geology)
- Configuration: Parameter selection (particle size, percentage passing), weathering color schemes, geological mappings
- Processing: Mapping extraction → CSV loading → formation grouping → outlier filtering (optional)
  → particle size distribution plot generation (lines per location, colored by weathering state) →
  investigation tracking (CSV summaries) → Excel export with highlighted outliers
- Output: Particle size distribution plots (log scale x-axis) + investigation CSV summaries + Excel exports
...
PSD plot generation with line plots per location (PHASE 4) → Investigation tracking (PHASE 5) →
...
- Configurable plotting behavior via CONFIG["investigation_series"]["average_by_investigation"]
- MODE 1 (average_by_investigation=True, default): One averaged line per investigation
  - For each investigation, averages percentage passing values at each particle size
  - Provides cleaner visualization showing investigation-level trends
  - All lines shown with full opacity (alpha_labeled)
- MODE 2 (average_by_investigation=False): One line per sample (Location ID + Sample Reference)
  - Shows detailed sample-level variations within investigations
  - Each sample labeled with format "LocationID_SampleRef" (e.g., BH01_3)
  - First sample per investigation labeled in legend, others have reduced opacity
  - Useful for identifying outliers or sample-specific patterns
  - Optional sample ID labels at line start (toggle via CONFIG["investigation_series"]["location_id_labels"]["enabled"])
    - Labels positioned at leftmost point with configurable font, color, and offset
    - Only displays when average_by_investigation=False
- Both modes use investigation-based coloring (rotating palette)
- X-axis: log scale for particle size (mm), range 0.001 to 200
- Y-axis: linear scale for percentage passing (%), range 0 to 105
- Line styling: solid lines with configurable opacity
- Single legend listing all investigations
- Creates both with-outliers and without-outliers versions
- Publication-quality line plots (300 DPI, 10x8 aspect ratio)
...
# MODIFICATION POINT: Calculate base paths relative to script location
_SCRIPT_DIR = Path(__file__).parent
_BASE_DIR = _SCRIPT_DIR.parent  # Go up from "SESRO Plotting Scripts" to "Figures"
...
CONFIG = {
    ...
    # ═══════════════════════════════════════════════════════════════════════
    "parameter": {
        "name": "ParticleSizeDistribution",  # MODIFICATION POINT: Change to any parameter
        "display_name": "Particle Size (mm)",  # For X-axis label
        "mapping_csv": str(
            _BASE_DIR / "Global_Parameter_Mapping_Extraction_only_CORRECTED.csv"
        ),
        "csv_source_folder": str(_BASE_DIR / "Openground CSVs"),
        "output_base_folder": str(_BASE_DIR / "Output"),
        # PSD-specific configuration
        "percentage_passing_column": "Percentage Passing",  # Y-axis data column
        "particle_size_column": "Particle Size",  # X-axis data column
        "sample_reference_column": "Sample Reference",  # Column containing sample identifiers (e.g., "1", "2", "3")
    },
    # ═══════════════════════════════════════════════════════════════════════
    ...
    "plotting": {
        "figure": {
            "width": 10,  # PSD plot width (matches matplotlib particle_size_distribution_by_formation.py)
            "height": 8,  # PSD plot height (matches matplotlib particle_size_distribution_by_formation.py)
            "dpi": 300,
        },
        "colors": {
            # PSD weathering colors (matches matplotlib script)
            "weathered_color": "#3C3C3C",  # Dark Gray for weathered
            "unweathered_color": "#00b3ff",  # Blue for unweathered
        },
        "axes": {
            # PSD-specific axis configuration (matches matplotlib script)
            "x_label": "Particle Size (mm)",
            "x_label_size": 10,
            "x_label_weight": "normal",
            "x_label_pad": 50,  # MODIFICATION POINT: Vertical spacing between x-axis and label (default: 4)
            "x_scale": "log",  # CRITICAL: Log scale for particle size
            "x_limit_left": 0.001,  # Minimum particle size (mm)
            "x_limit_right": 200,  # Maximum particle size (mm)
            "x_ticks": [0.001, 0.01, 0.1, 1, 10, 100, 200],  # Log scale ticks
            "y_label": "Percentage Passing (%)",
            "y_label_size": 10,
            "y_label_weight": "normal",
            "y_limit_bottom": 0,  # Start at 0%
            "y_limit_top": 105,  # End at 105%
            "y_ticks_spacing": 10,  # Y-axis tick spacing (every 10%)
            "invert_y": False,  # Normal Y-axis (not inverted like depth plots)
            "grid": True,
            "grid_which": "both",
            "grid_style": "-",
            "grid_width": 0.5,
        ...
        "legend": {
            "location": "lower right",  # PSD legend position (matches matplotlib script)
            "edgecolor": "black",
            ...
            "bbox_to_anchor_y": -0.17,  # Vertical position below plot (centered horizontally)
        },
        "save": {
            "bbox_inches": "tight",
        },
        "lines": {
            # Line styling for PSD plots
            "linestyle": "-",  # Solid lines
            "linewidth": 2,  # Line thickness
            "alpha_labeled": 1,  # Full opacity for labeled lines (weathered/unweathered)
            "alpha_unlabeled": 0.5,  # Transparency for non-labeled location lines
        },
    },
    # ═══════════════════════════════════════════════════════════════════════
    ...
    "investigation_series": {
        ...
        "average_by_investigation": False,  # MODIFICATION POINT: True=one averaged line per investigation, False=one line per location
        "location_id_labels": {
            "enabled": True,  # MODIFICATION POINT: Show location ID labels at start of lines (only applies when average_by_investigation=False)
            "font_size": 5,  # Font size for location ID labels
            "font_weight": "normal",  # Font weight: "normal", "bold", etc.
            "color": "black",  # Label text color
            "alpha": 1,  # Label transparency (0-1)
            "offset_x": -0.05,  # Horizontal offset from line start (as fraction of x-axis range in log space)
            "offset_y": 0,  # Vertical offset from line start (in percentage passing units)
        },
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # PARTICLE SIZE CLASSIFICATION CONFIGURATION
    # BS EN ISO 14688-1:2018 soil classification system
    # ═══════════════════════════════════════════════════════════════════════
    "particle_classification": {
        "enabled": True,  # MODIFICATION POINT: Enable/disable classification legend below plot
        "subplot_height_ratio": 0.08,  # Height ratio for classification subplot (relative to main plot)
        "spacing": 0.1,  # Vertical spacing between main plot and classification legend
        "font_size": 9,  # Font size for classification labels
        "font_weight": "bold",
        "line_width": 0.8,  # Width of boundary lines (matches main plot axis)
        "line_color": "black",
        # Classification structure with size boundaries in mm
        "categories": [
            {
                "name": "CLAY",
                "size_min": 0.001,  # Left boundary for CLAY box
                "size_max": 0.002,
                "label": "< 0.002",
                "subcategories": None,
            },
            {
                ...
                "size_min": 0.002,
                "size_max": 0.06,
                "label": None,  # Will show subcategory labels
                "subcategories": [
...
def _create_description_from_code(
    ...
    if geology_code_col and geology_code_col in df_copy.columns:
        ...
        df_copy["Geological_Strata"] = df_copy["Geological_Strata"].fillna(
            df_copy[geology_code_col]
        )
...
def create_enhanced_geological_strata_column(
    ...
    # Note: geology_code2_desc_col is OPTIONAL for formation splitting
    # The classification logic only requires geology_code2 (e.g., "KC_UW", "KC_W")
    # If missing, it will be None but formation splitting can still proceed
    if (
        geology_code2_desc_col is None
        or geology_code2_desc_col not in df_enhanced.columns
    ):
        logger.debug(
            f"INFO {csv_name}: No Geology Code 2 Description column found, will use Code 2 only"
        )
        # Don't return early - continue with formation splitting using geology_code2
...
def create_sample_identifier(
    location_id: str, sample_reference: Any, depth_top: Optional[float] = None
) -> str:
    """
    Create sample identifier string in format 'LocationID_SampleRef' or 'LocationID_Depth'.

    Combines location ID and sample reference into a single identifier string
    for labeling individual samples in plots (e.g., "BH01_3"). If sample reference
    is missing or empty, falls back to using depth_top (e.g., "BH01_12.5").

    Args:
        location_id: Location identifier (e.g., "BH01", "BH18")
        sample_reference: Sample reference number or identifier (e.g., 1, "3", "7")
        depth_top: Optional depth value to use as fallback when sample_reference is missing

    Returns:
        Formatted sample identifier string (e.g., "BH01_3" or "BH01_12.5")

    Example:
        >>> create_sample_identifier("BH01", 3)
        "BH01_3"
        >>> create_sample_identifier("BH18", "7")
        "BH18_7"
        >>> create_sample_identifier("BH09", None, 12.5)
        "BH09_12.5"
        >>> create_sample_identifier("BH09", "", 12.5)
        "BH09_12.5"
    """
    # Convert sample_reference to string and handle empty/NaN values
    sample_ref_str = str(sample_reference).strip() if pd.notna(sample_reference) else ""
    if not sample_ref_str or sample_ref_str.lower() == "nan":
        # Fallback to depth_top if sample reference is missing
        if depth_top is not None and pd.notna(depth_top):
            return f"{location_id}_{depth_top}"
        return str(
            location_id
        )  # Return just location ID if no sample reference or depth
    return f"{location_id}_{sample_ref_str}"


def add_particle_size_classification_legend(
    classification_ax: Any,
    classification_config: Dict[str, Any],
    x_limit_left: float,
    x_limit_right: float,
) -> None:
    """
    Add particle size classification legend in a separate subplot with its own axes.

    Creates a visual classification system showing soil particle size categories
    (CLAY, SILT, SAND, GRAVEL, COBBLES, BOULDERS) with their size ranges in mm.
    The classification legend has its own top and bottom boundaries (axes).

    Args:
        classification_ax: Matplotlib Axes object for classification subplot
        classification_config: Configuration dict with classification categories
    ...
    font_size = classification_config["font_size"]
    font_weight = classification_config["font_weight"]
    line_width = classification_config["line_width"]
    line_color = classification_config["line_color"]
    ...
    # Remove tick marks from bottom boundary
    ...
    for category in categories:
        ...
        # Draw subcategories or single size range label
        if subcategories:
            ...
            for subcat in subcategories:
                ...
                # Draw subdivision boundary line (lighter/thinner, top cell only)
                classification_ax.axvline(
                    x=sub_min,
                    ymin=0.5,
                    ymax=1.0,
            ...
            # Single category without subdivisions - show at top
    ...
    classification_ax.axhline(
        y=0, xmin=0, xmax=1, color=line_color, linewidth=line_width
    )

    # Draw horizontal line between subdivisions (top) and main categories (bottom)
    # Only spans from silt start (0.002 mm) to gravel end (60 mm)
    classification_ax.plot(
        [0.002, 60], [0.5, 0.5], color=line_color, linewidth=line_width
    )

    # Draw left boundary line at start of silt (0.002 mm)
    classification_ax.axvline(
        x=0.002, ymin=0, ymax=1, color=line_color, linewidth=line_width
    )
...
def generate_investigation_series_plot(
    ...
    """
    Generate particle size distribution plot with configurable averaging behavior.

    Creates PSD line plot with two modes controlled by CONFIG["investigation_series"]["average_by_investigation"]:

    MODE 1 (average_by_investigation=True, default):
    - One line per Investigation (averaged across all locations)
    - For each investigation, percentage passing values are averaged at each particle size
    - Provides cleaner visualization showing investigation-level trends
    - All lines shown with full opacity (alpha_labeled)

    MODE 2 (average_by_investigation=False):
    - One line per Sample (Location ID + Sample Reference, colored by investigation)
    - Shows detailed sample-level variations within investigations
    - Each sample labeled with format "LocationID_SampleRef" (e.g., BH01_3)
    - First sample per investigation labeled in legend, others have reduced opacity
    - Useful for identifying outliers or sample-specific patterns
    - Optional sample ID labels at the start of each line (toggle via CONFIG["investigation_series"]["location_id_labels"]["enabled"])
      * Labels positioned at leftmost point with configurable offset
      * Font size, weight, color, and transparency configurable
      * Only applies when average_by_investigation=False

    Both modes use:
    - Lines colored by investigation (rotating palette from CONFIG)
    - X-axis: Particle Size (mm) - log scale
    ...
    """
    logger.info(f"🎨 Generating PSD plot for formation: {formation_name}")
    ...
    # PSD-specific axes config
    x_label = CONFIG["plotting"]["axes"]["x_label"]
    x_label_size = CONFIG["plotting"]["axes"]["x_label_size"]
    x_label_weight = CONFIG["plotting"]["axes"]["x_label_weight"]
    x_label_pad = CONFIG["plotting"]["axes"]["x_label_pad"]
    x_scale = CONFIG["plotting"]["axes"]["x_scale"]
    x_limit_left = CONFIG["plotting"]["axes"]["x_limit_left"]
    x_limit_right = CONFIG["plotting"]["axes"]["x_limit_right"]
    ...
    y_limit_bottom = CONFIG["plotting"]["axes"]["y_limit_bottom"]
    y_limit_top = CONFIG["plotting"]["axes"]["y_limit_top"]
    y_ticks_spacing = CONFIG["plotting"]["axes"]["y_ticks_spacing"]
    ...
    grid_alpha = CONFIG["plotting"]["axes"]["grid_alpha"]
    ...
    # Line styling
    linestyle = CONFIG["plotting"]["lines"]["linestyle"]
    linewidth = CONFIG["plotting"]["lines"]["linewidth"]
    alpha_labeled = CONFIG["plotting"]["lines"]["alpha_labeled"]
    alpha_unlabeled = CONFIG["plotting"]["lines"]["alpha_unlabeled"]
    ...
    # Location ID label configuration
    location_id_labels_enabled = CONFIG["investigation_series"]["location_id_labels"][
        "enabled"
    ]
    label_font_size = CONFIG["investigation_series"]["location_id_labels"]["font_size"]
    label_font_weight = CONFIG["investigation_series"]["location_id_labels"][
        "font_weight"
    ]
    label_color = CONFIG["investigation_series"]["location_id_labels"]["color"]
    label_alpha = CONFIG["investigation_series"]["location_id_labels"]["alpha"]
    label_offset_x = CONFIG["investigation_series"]["location_id_labels"]["offset_x"]
    label_offset_y = CONFIG["investigation_series"]["location_id_labels"]["offset_y"]

    particle_size_column = CONFIG["parameter"]["particle_size_column"]
    percentage_passing_column = CONFIG["parameter"]["percentage_passing_column"]
    sample_reference_column = CONFIG["parameter"]["sample_reference_column"]

    # === FIGURE INITIALIZATION WITH GRIDSPEC ===
    # Check if classification legend is enabled to determine subplot layout
    classification_config = CONFIG.get("particle_classification", {})
    classification_enabled = classification_config.get("enabled", False)

    if classification_enabled:
        # Create figure with GridSpec for main plot + classification legend
        from matplotlib.gridspec import GridSpec

        classification_height_ratio = classification_config.get(
            "subplot_height_ratio", 0.12
        )
        classification_spacing = classification_config.get("spacing", 0.08)

        # Height ratios: main plot gets 1.0, classification gets its ratio
        height_ratios = [1.0, classification_height_ratio]

        fig = plt.figure(figsize=(fig_width, fig_height))
        gs = GridSpec(
            2, 1, figure=fig, height_ratios=height_ratios, hspace=classification_spacing
        )

        ax = fig.add_subplot(gs[0])  # Main plot
        classification_ax = fig.add_subplot(gs[1])  # Classification legend
    else:
        # Standard single subplot
        fig, ax = plt.subplots(figsize=(fig_width, fig_height))
        classification_ax = None

    # === DATA COLLECTION AND PROCESSING ===
    # Collect all data from all CSVs first, then plot lines per location
    all_data = []

    for csv_name, df in csv_dict.items():
        # Find parameter column (should be "Percentage Passing")
        matching_mappings = parameter_mappings[
            parameter_mappings["csv_file"] == csv_name
        ]
        if matching_mappings.empty:
        ...
        # Verify required columns exist
        if (
            particle_size_column not in df.columns
            or percentage_passing_column not in df.columns
        ):
            logger.warning(
                f"⚠️ {csv_name}: Missing PSD columns (need '{particle_size_column}' and '{percentage_passing_column}')"
            )
            continue

        if "Location ID" not in df.columns:
            logger.warning(f"⚠️ {csv_name}: Missing 'Location ID' column")
            continue
        ...
        all_data.append(df_with_inv)

    # Combine all data from different CSVs
    if not all_data:
        logger.warning(f"⚠️ No data available for {formation_name}")
        plt.close(fig)
        return

    combined_df = pd.concat(all_data, ignore_index=True)

    # Collect unique investigations and create color mapping
    all_investigations = sorted(combined_df["Investigation"].unique())
    investigation_color_map = create_investigation_color_mapping(
        all_investigations, investigation_colors
    )

    # Read averaging toggle from CONFIG
    average_by_investigation = CONFIG["investigation_series"][
        "average_by_investigation"
    ]

    # === PLOTTING SECTION ===
    if average_by_investigation:
        # === MODE 1: ONE AVERAGED LINE PER INVESTIGATION ===
        # Group by Investigation and average percentage passing at each particle size
        for investigation in sorted(all_investigations):
            inv_data = combined_df[combined_df["Investigation"] == investigation].copy()

            if inv_data.empty:
                continue

            # Group by particle size and calculate mean percentage passing
            # This averages all measurements at each particle size within this investigation
            averaged_data = (
            ...
            # Sort by particle size for proper line plotting
            averaged_data = averaged_data.sort_values(by=particle_size_column)

            # Extract particle size and averaged percentage passing
            ...
            # Get color for this investigation
            inv_color = investigation_color_map[investigation]

            # Plot one averaged line per investigation with full opacity
            ax.plot(
                particle_sizes,
                percentage_passing,
                linestyle=linestyle,
                linewidth=linewidth,
                color=inv_color,
                alpha=alpha_labeled,  # Full opacity for investigation-level averages
                label=investigation,
            )
    else:
        # === MODE 2: ONE LINE PER SAMPLE (COLORED BY INVESTIGATION) ===
        # Track which investigations have been labeled for legend
        labeled_investigations = set()

        # Create Sample_ID column combining Location ID and Sample Reference (with depth fallback)
        combined_df["Sample_ID"] = combined_df.apply(
            lambda row: create_sample_identifier(
                row["Location ID"],
                row.get(sample_reference_column, ""),
                row.get("Top Depth"),
            ),
            axis=1,
        )
        ...
        for investigation in sorted(all_investigations):
            ...
            unique_samples = sorted(inv_data["Sample_ID"].unique())

            for sample_id in unique_samples:
                sample_data = inv_data[inv_data["Sample_ID"] == sample_id].copy()
                ...
                # Extract particle size and percentage passing
                ...
                if investigation not in labeled_investigations:
                    ...
                    # Unlabeled lines (same investigation)
                    ax.plot(
                        particle_sizes,
                        percentage_passing,
                        linestyle=linestyle,
                        linewidth=linewidth,
                        color=inv_color,
                        alpha=alpha_unlabeled,  # Reduced opacity for additional lines
                    )

                # Add sample ID label at the start of the line if enabled
                if location_id_labels_enabled and len(particle_sizes) > 0:
                    # Get the leftmost point (smallest particle size)
                    start_x = particle_sizes[0]
                    start_y = percentage_passing[0]

                    # Calculate offset in log space for x
                    # offset_x is a fraction of the log range
                    log_x_min = np.log10(x_limit_left)
                    log_x_max = np.log10(x_limit_right)
                    log_x_range = log_x_max - log_x_min
                    log_x_offset = log_x_min + (label_offset_x * log_x_range)
                    x_pos = 10**log_x_offset

                    # Y offset is simple linear offset
                    y_pos = start_y + label_offset_y

                    # Add text label with sample identifier
                    ax.text(
                        x_pos,
                        y_pos,
                        str(sample_id),
    ...
    # === AXIS CONFIGURATION ===
    # Set X-axis (Particle Size) - LOG SCALE
    ax.set_xscale(x_scale)
    ax.set_xlim(left=x_limit_left, right=x_limit_right)
    ax.set_xticks(x_ticks)
    ax.get_xaxis().set_major_formatter(plt.ScalarFormatter())

    # Set Y-axis (Percentage Passing)
    ax.set_ylim(bottom=y_limit_bottom, top=y_limit_top)
    ax.set_yticks(np.arange(y_limit_bottom, y_limit_top + 1, y_ticks_spacing))

    # Set axis labels
    ax.set_xlabel(
        x_label, fontsize=x_label_size, weight=x_label_weight, labelpad=x_label_pad
    )
    ax.set_ylabel(y_label, fontsize=y_label_size, weight=y_label_weight)

    # Add plot title
    ax.set_title(
        formation_name, fontsize=title_fontsize, weight=title_weight, pad=title_pad
    )
    ...
    # === LEGEND ===
    # Move legend much lower to make space for classification legend
    # Adjust bbox_to_anchor_y based on whether classification is enabled
    if classification_enabled:
        # Position legend lower when classification is present
        legend_y_position = -0.35  # Much lower to accommodate classification legend
    else:
        # Standard position when no classification
        legend_y_position = CONFIG["plotting"]["legend"].get("bbox_to_anchor_y", -0.17)
    ...
    # === ADD PARTICLE SIZE CLASSIFICATION LEGEND ===
    if classification_enabled and classification_ax is not None:
        add_particle_size_classification_legend(
            classification_ax, classification_config, x_limit_left, x_limit_right
        )
    ...
    logger.info(f"✅ Saved PSD plot: {output_path}")
